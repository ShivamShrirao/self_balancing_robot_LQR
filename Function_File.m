...
function eqbm_points = find_equilibrium_points(x1_dot, x2_dot)
  ...
  x2_dot == 0;
  ################## ADD YOUR CODE HERE ######################
  eqbm_points = solve(x1_dot,x2_dot);         ## Solve gives the roots of the equations.
  ############################################################  
endfunction
...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  ...
  ################## ADD YOUR CODE HERE ######################
  for k = 1:length(eqbm_points)
    jacobian_matrices{k} = double(subs(jacobian([x1_dot,x2_dot]),{x1,x2},solutions{k}));      ## Calculate jacobian and substitute values in it.
  endfor
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(eqbm_pts, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    ################## ADD YOUR CODE HERE ######################
    eigen_values{k}=eig(matrix);            # calculate eigen values
    if any(real(eigen_values{k})>0)         # check if any of the real value is positive
      flag = 0;                             # If any value is positive set flag 0 for system is unstable.
    endif
    ############################################################
    if flag == 1
