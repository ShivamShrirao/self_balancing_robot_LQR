...
function dy = cart_pendulum_dynamics(y, m, M, L, g, u)
  ...
  cos_theta=cos(y(3));
  dy(1,1) = y(2);
  dy(2,1) = (u + m*L*sin_theta*y(4)^2 + m*g*sin_theta*cos_theta)/(M+m*sin_theta^2);
  dy(3,1) = y(4);
  dy(4,1) = (u*cos_theta - (M+m)*g*sin_theta - m*L*cos_theta*sin_theta*y(4)^2)/(L*(M+m*sin_theta^2));
endfunction
