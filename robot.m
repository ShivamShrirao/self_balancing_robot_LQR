...
##          govern this system.
function dy = cart_pendulum_dynamics(y, m, M, L, Inr, g, u)
  sin_theta=sin(y(3));
  cos_theta=cos(y(3));
  dy(1,1) = y(2);
  dy(2,1) = ((u - m*L*sin_theta*y(4)^2)*(L+Inr/(m*L)) + m*g*L*sin_theta*cos_theta)/((M+m)*Inr/(m*L) + L*(M+m*sin_theta^2));
  dy(3,1) = y(4);
  dy(4,1) = (u*cos_theta + (M+m)*g*sin_theta - m*L*cos_theta*sin_theta*y(4)^2)/((M+m)*Inr/(m*L) + L*(M+m*sin_theta^2));
endfunction
...
function [t,y] = sim_cart_pendulum(m, M, L, Inr, g, y0)
  tspan = 0:0.1:10;                  ## Initialise time step           
  u = 0;                             ## No Input
  [t,y] = ode45(@(t,y)cart_pendulum_dynamics(y, m, M, L, Inr, g, u),tspan,y0); ## Solving the differential equation    
endfunction
...
function [A, B] = cart_pendulum_AB_matrix(m, M, L, Inr, g)
  A = [0 1 0 0 ; 0 0 m*L*g/(M*L+Inr*(M+m)/(m*L)) 0 ; 0 0 0 1 ; 0 0 g*(M+m)/(M*L+Inr*(M+m)/(m*L)) 0];
  B = [0 ; (Inr/(m*L) + L)/(M*L+Inr*(M+m)/(m*L)) ; 0 ; 1/(M*L+Inr*(M+m)/(m*L))];
endfunction
...
function [t,y] = pole_place_cart_pendulum(m, M, L, Inr, g, y_setpoint, y0)
  [A,B] = cart_pendulum_AB_matrix(m, M, L, Inr, g);  ## Initialize A and B matrix 
  eigs = [-3;-3;-3;-3];                    ## Initialise desired eigenvalues
  K = place(A,B,eigs);
  disp(K);
  
  tspan = 0:0.1:10;
  [t,y] = ode45(@(t,y)cart_pendulum_dynamics(y, m, M, L, Inr, g, -K*(y-y_setpoint)),tspan,y0);
endfunction
...
function [t,y] = lqr_cart_pendulum(m, M, L, Inr, g, y_setpoint, y0)
  global A, global B, global K, global Q, global R
  [A,B] = cart_pendulum_AB_matrix(m, M, L, Inr, g);  ## Initialize A and B matrix 
  Q = diag([4,3,10,8]);
  R = 1;
...
##  [t,y] = sim_cart_pendulum(m, M, L, Inr, g, y0);
##  [t,y] = pole_place_cart_pendulum(m, M, L, Inr, g, y_setpoint, y0);
  [t,y] = lqr_cart_pendulum(m, M, L, Inr, g, y_setpoint, y0);
##  for k = 1:length(t)
